...
@section('content')
    <div class="panel mt-10">
        <div class="flex items center justify-between">
            ...
            <a href="{{ route('role.create') }}" class="btn btn-primary btn-sm py-2.5"><i data-lucide="Plus"
                    class="h-4"></i>Tambah
                data</a>
        </div>
        <!-- hover table -->
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td class="px-2 py-5">{{ $item->nama }}</td>
                            <td class="px-2 py-5 text-center flex items-center justify-center gap-x-2">
                                <a href="{{ route('role.edit', ['role' => $item->role_id]) }}" x-tooltip="Edit"
                                    class="mb-1">
                                ...
                                <div x-data="{ open: false }" x-init="window.addEventListener('open-delete-modal', e => {
                                    ...
                                    const action = '{{ route('role.destroy', ':id') }}'.replace(':id', e.detail.dataId);
                                    form.setAttribute('action', action);
                                })">
                                    <!-- Tombol Hapus -->
                                    <button type="button" x-tooltip="Delete"
                                        @click="$dispatch('open-delete-modal', { dataId: '{{ $item->role_id }}' })">
