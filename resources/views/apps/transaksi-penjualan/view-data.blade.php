...
@section('content')
    <div class="panel mt-10">
        <div class="flex items center justify-between">
            ...
            <a href="{{ route('transaksi-penjualan.create') }}" class="btn btn-primary btn-sm py-2.5"><i data-lucide="Plus"
                    class="h-4"></i>Tambah
                data</a>
        </div>
        <!-- hover table -->
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td class="px-2 py-5 font-semibold hover:underline hover:text-sky-700"> <a
                                        href="{{ route('transaksi-penjualan.create', ['kode_transaksi' => $item->kode_transaksi]) }}">
                                        {{ $item->kode_transaksi }} </a></td>
                                <td class="px-2 py-5">Rp {{ number_format($item->total_bayar, 0, ',', '.') }}</td>
                                ...
                                <td class="px-2 py-5 text-center flex items-center justify-center gap-x-2">
                                    ...
                                    <div x-data="{ open: false }" x-init="window.addEventListener('open-delete-modal', e => {
                                        ...
                                        <button type="button" x-tooltip="Delete"
                                            @click="$dispatch('open-delete-modal', { dataId: '{{ $item->kode_transaksi }}' })">
