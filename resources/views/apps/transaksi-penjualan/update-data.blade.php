    ...
    <div class="panel mt-10">
        <form method="POST" action="{{ route('master-barang.update', ['master_barang' => $data->kode_barang]) }}">
            ...
            <div class="grid grid-cols-4 gap-5 mt-8">
                ...
                <div class="relative @error('satuan') has-error @enderror">
                    <p class="font-semibold">Satuan</p>
                    <select id="seachable-select" name="satuan" class="absolute top-1">
                        <option value="">Pilih satuan</option>
                        @foreach ($satuan as $item)
                            <option value="{{ $item->nama_satuan }}"
                                {{ $data->satuan == $item->nama_satuan ? 'selected' : '' }}>{{ $item->nama_satuan }}
                            </option>
                        @endforeach
                    </select>
                    @error('satuan')
