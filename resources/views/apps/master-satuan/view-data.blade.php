...
@section('content')
    <div class="panel mt-10">
        <div class="flex items center justify-between">
            ...
            <a href="{{ route('master-satuan.create') }}" class="btn btn-primary btn-sm py-2.5"><i data-lucide="Plus"
                    class="h-4"></i>Tambah
                data</a>
        </div>
        <!-- hover table -->
            ...
            <table class="sticky-header w-full text-sm">
                <thead class="bg-red-600">
                    <tr class="space-y-2">
                        ...
                        <th class="px-2 py-3">Nama satuan</th>
                        <th class="px-2 py-3 text-center">Stok minimal</th>
                ...
                <tbody>
                    @if ($data->isNotEmpty())
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td class="px-2 py-5">{{ $item->nama_satuan }}</td>
                                <td class="px-2 py-5 text-center">{{ $item->stok_minimal }}</td>
                                <td class="px-2 py-5"><span
                                        class="font-semibold badge {{ $item->status == 'Aktif' ? 'bg-success' : 'bg-danger' }}">{{ $item->status }}</span>
                                </td>
                                <td class="px-2 py-5">{{ $item->remark ? $item->remark : '-' }}</td>
                                <td class="px-2 py-5 text-center flex items-center justify-center gap-x-2">
                                    <a href="{{ route('master-satuan.edit', ['master_satuan' => $item->id]) }}"
                                        x-tooltip="Edit" class="mb-1">
                                    ...
                                    <div x-data="{ open: false }" x-init="window.addEventListener('open-delete-modal', e => {
                                        ...
                                        const action = '{{ route('master-satuan.destroy', ':id') }}'.replace(':id', e.detail.dataId);
                    ...
                    @else
                        <tr>
                            <td colspan="5" class="text-center text-sm py-3">Data tidak ditemukan</td>
                        </tr>
                    @endif
