...
@section('content')
    <div class="panel mt-10">
        <div class="flex items center justify-between">
            ...
            <a href="{{ route('master-barang.create') }}" class="btn btn-primary btn-sm py-2.5"><i data-lucide="Plus"
                    class="h-4"></i>Tambah
                data</a>
        </div>
        <!-- hover table -->
            ...
            <table class="sticky-header w-full text-sm">
                <thead class="bg-red-600">
                    <tr class="space-y-2">
                        ...
                        <th class="px-2 py-3">Kode barang</th>
                        <th class="px-2 py-3">Nama barang</th>
                        <th class="px-2 py-3 text-center">Stok</th>
                        <th class="px-2 py-3 text-center">Harga beli</th>
                        <th class="px-2 py-3 text-center">harga jual</th>
                    ...
                    @if ($data->isNotEmpty())
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td class="px-2 py-5">{{ $item->kode_barang }}</td>
                                <td class="px-2 py-5">{{ $item->nama_barang }}</td>
                                <td class="px-2 py-5 text-center">{{ number_format($item->stok, 0, ',', '.') }}</td>
                                ...
                                <td class="px-2 py-5 text-center flex items-center justify-center gap-x-2">
                                    <a href="{{ route('master-barang.edit', ['master_barang' => $item->kode_barang]) }}"
                                    ...
                                    <div x-data="{ open: false }" x-init="window.addEventListener('open-delete-modal', e => {
                                        ...
                                        const action = '{{ route('master-barang.destroy', ':id') }}'.replace(':id', e.detail.dataId);
                                        form.setAttribute('action', action);
                                    })">
                                        <!-- Tombol Hapus -->
                                        <button type="button" x-tooltip="Delete"
                                            @click="$dispatch('open-delete-modal', { dataId: '{{ $item->kode_barang }}' })">
